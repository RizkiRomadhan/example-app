                                    ...
                                    <tbody>
                                        @forelse ($lab as $lb)
                                            <tr>
                                                ...
                                                <td>{{ $lb->nama_mahasiswa }}</td>
                                                <td>{{ $lb->nama_lab }}</td>
                                                <td>{{ $lb->nama_laboran }}</td>
                                                <td>{{ $lb->waktu }}</td>
                                                <td>{{ $lb->status }}</td>
                                                <td style="display: flex;">
                                                    <form action="{{ route('konfirmasi-lab.update', $lb->id) }}" method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        <button type="submit" class="btn btn-warning">Diterima</button>
                                                    </form>
                                                    <form action="{{ route('konfirmasi-lab.destroy', $lb->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')

                                                        <button type="submit" class="btn btn-danger" style="margin-left: 1em;">Ditolak</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @empty
